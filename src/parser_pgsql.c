...
static void skip_to_semicolon_pg(Lexer *lex) {
    ...
    for (;;) {
        ...
        if (t == TOK_SEMICOLON || t == TOK_EOF) return;
    }
}
...
static AppResult parse_create_table_pg(Lexer *lex, TableRegistry *reg,
    ...
    if (!table_name) return ERR_MEMORY;
    ...
    while (depth > 0) {
        ...
        if (t == TOK_KW_PRIMARY || t == TOK_KW_UNIQUE || t == TOK_KW_CHECK ||
            ...
            for (;;) {
                t = lexer_next(lex, &tok);
                if (t == TOK_EOF) goto done;
                if (t == TOK_LPAREN) inner_depth++;
                else if (t == TOK_RPAREN) {
                    if (inner_depth > 0) inner_depth--;
                    else { depth--; goto done; }
                }
                else if (t == TOK_COMMA && inner_depth == 0) break;
            }
            continue;
        }
    ...
    if (col_count > 0) {
        ...
        if (res != OK) {
            for (int i = 0; i < col_count; i++) free(columns[i]);
            free(table_name);
            return res;
        }
...
static AppResult parse_copy_pg(Lexer *lex, TableRegistry *reg, ProgressCtx *prog) {
    ...
    if (t == TOK_LPAREN) {
        for (;;) {
            t = lexer_next(lex, &tok);
            if (t == TOK_RPAREN) break;
            if (t == TOK_COMMA) continue;
    ...
    if (t != TOK_KW_FROM) {
        for (int i = 0; i < copy_col_count; i++) free(copy_columns[i]);
        free(table_name);
        skip_to_semicolon_pg(lex);
        return OK;
    }
    ...
    if (!table) {
        ...
        for (;;) {
            t = lexer_next(lex, &tok);
            if (t == TOK_COPY_END || t == TOK_EOF) break;
        }
                                ...
                                for (size_t j = 0; j < flen; j++) {
                                    if (field[j] == '\\' && j + 1 < flen) {
                                        ...
                                        switch (field[j]) {
                                            ...
                                            default:   decoded[dlen++] = field[j]; break;
                                        }
                                    } else {
...
static AppResult parse_insert_pg(Lexer *lex, TableRegistry *reg,
    ...
    if (t == TOK_LPAREN) {
        ...
        while (depth > 0) {
            ...
            else if (t == TOK_EOF) { free(table_name); return OK; }
        }
        t = lexer_next(lex, &tok);
    }
    ...
    for (;;) {
        t = lexer_next(lex, &tok);
        if (t == TOK_SEMICOLON || t == TOK_EOF) break;
        if (t == TOK_COMMA) continue;

        if (t != TOK_LPAREN) continue;
        ...
        for (;;) {
            t = lexer_next(lex, &tok);
            if (t == TOK_RPAREN || t == TOK_EOF) break;
            if (t == TOK_COMMA) continue;
